...
class PathGather(nn.Module):
    def __init__(self, embedding_size,ablation_type, max_paths=128):
    ...
    def forward(self, link_user_embeddings, link_answer_embeddings):
        ...
        if self.ablation_type=='ua':
            score[:,1:]=0
        elif self.ablation_type=='utqa':
            score[:,0]=0
            score[:,4:]=0
        elif self.ablation_type=='uqtqa':
            score[:,:4]=0
...
class ExplainableGraphTransformer(nn.Module):
    def __init__(self, embeddingDict, embedding_size, etypes, max_length, num_head, ablation_type,device='cuda:0', max_paths=10):
        super().__init__()
        ...
        self.pathgather = PathGather(embedding_size, ablation_type,max_paths)
