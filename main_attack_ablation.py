...
import pickle
import warnings
from lp_models.MetaTransformer_ablation import ExplainableGraphTransformer,RGCNClassfier
import numpy as np
import scipy.sparse as sp
import torch
from sklearn.metrics import roc_auc_score
import torch.optim as optim
from utils.io_ablation import load_data
import tqdm
import copy
import datetime
import gc
...
def train(model, g, train_pos_loader, train_neg_loader, Answers_dev_dataloader,Answers_test_dataloader,optimizer, epoch):
    ...
    for epoch in range(epoch):
        ...
        if mrr_dev>best_mrr:
            ...
            torch.save(model.state_dict(), os.path.join(args.outputs, f'best_mrr_model_{args.dataset}_{args.ablation_type}.pt'))
...
if __name__ == '__main__':
    logging.basicConfig(filename='log_attack_ablation.txt', level=logging.CRITICAL)
    print('-'*30, 'NEW EXP', '-'*30)
    ...
    parser.add_argument('--ablation_type', type=str, default='ua', help="[ua,uqta,uqtqa]")
    ...
    if args.lp_model == 'metatransformer' or args.lp_model == 'rgcn':
        ...
        if args.lp_model == 'rgcn':
            ...
            model = ExplainableGraphTransformer(embeddingDict,128, g.etypes,max_length=5,num_head=8,ablation_type=args.ablation_type,device=args.device,max_paths=10)
