...
if __name__ == '__main__':
    ...
    data['val_mrr']=data.best_val_result.apply(lambda x:round(x[-1]*100,2))
    data['test_mrr']=data.best_test_result.apply(lambda x:round(x[-1]*100,2))
    
    clean=data.groupby(['dataset','lp_model','exp_type','attack_method','attack_goal','attack_rate'])[['val_auc','test_auc','val_mrr','test_mrr']].mean()
