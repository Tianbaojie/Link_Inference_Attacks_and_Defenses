...
class GCNII(torch.nn.Module):
    ...
    def forward(self,x, edge_index):
        ...
        for conv in self.convs:
            x = conv(x=x,x_0=x_0, edge_index=edge_index)
            x = F.relu(x)
            x=self.dropout(x)
...
class GCNII_LP(BaseLp):
    '''
    GCNII_LP model used for link prediction based on GCNII, and the output of the last layer is normalized.
    The model trains the GCNII model and then uses the output of the last layer as the node representation.
    ...
    '''
    def __init__(self,embedding_dim_in,embedding_dim_out,device,num_layers=5):
        super(GCNII_LP,self).__init__()
        self.device=device
        self.model=GCNII(embedding_dim_in,embedding_dim_out,num_layers=num_layers, alpha=0.1, theta=0.5,
            shared_weights=True, dropout=0.15).to(device)
